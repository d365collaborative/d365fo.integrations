Describe "Get-D365ODataEntityKey Unit Tests" -Tag "Unit" {
	BeforeAll {
	...
	Describe "Ensuring unchanged command signature" {
		It "should have the expected parameter sets" {
			(Get-Command Get-D365ODataEntityKey).ParameterSets.Name | Should -Be '__AllParameterSets'
		}
		
		It 'Should have the expected parameter Name' {
			$parameter = (Get-Command Get-D365ODataEntityKey).Parameters['Name']
		...
		It 'Should have the expected parameter Properties' {
			$parameter = (Get-Command Get-D365ODataEntityKey).Parameters['Properties']
