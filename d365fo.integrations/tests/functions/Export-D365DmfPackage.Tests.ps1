Describe "Export-D365DmfPackage Unit Tests" -Tag "Unit" {
	BeforeAll {
	...
	Describe "Ensuring unchanged command signature" {
		It "should have the expected parameter sets" {
			(Get-Command Export-D365DmfPackage).ParameterSets.Name | Should -Be '__AllParameterSets'
		}
		
		It 'Should have the expected parameter Path' {
			$parameter = (Get-Command Export-D365DmfPackage).Parameters['Path']
		...
		It 'Should have the expected parameter JobId' {
			$parameter = (Get-Command Export-D365DmfPackage).Parameters['JobId']
			$parameter.Name | Should -Be 'JobId'
			$parameter.ParameterType.ToString() | Should -Be System.String
			$parameter.IsDynamic | Should -Be $False
			$parameter.ParameterSets.Keys | Should -Be '__AllParameterSets'
			$parameter.ParameterSets.Keys | Should -Contain '__AllParameterSets'
		...
		It 'Should have the expected parameter Tenant' {
			$parameter = (Get-Command Export-D365DmfPackage).Parameters['Tenant']
		...
		It 'Should have the expected parameter Url' {
			$parameter = (Get-Command Export-D365DmfPackage).Parameters['Url']
		...
		It 'Should have the expected parameter ClientId' {
			$parameter = (Get-Command Export-D365DmfPackage).Parameters['ClientId']
		...
		It 'Should have the expected parameter ClientSecret' {
			$parameter = (Get-Command Export-D365DmfPackage).Parameters['ClientSecret']
		...
		It 'Should have the expected parameter EnableException' {
			$parameter = (Get-Command Export-D365DmfPackage).Parameters['EnableException']
	...
	Describe "Testing parameterset __AllParameterSets" {
		<#
		__AllParameterSets -Path -JobId
		__AllParameterSets -Path -JobId -Tenant -Url -ClientId -ClientSecret -EnableException
		#>
	}
