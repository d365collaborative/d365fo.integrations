Describe "Get-D365ODataMetadata Unit Tests" -Tag "Unit" {
	BeforeAll {
	...
	Describe "Ensuring unchanged command signature" {
		It "should have the expected parameter sets" {
			(Get-Command Get-D365ODataMetadata).ParameterSets.Name | Should -Be '__AllParameterSets'
		}
		
		It 'Should have the expected parameter Path' {
			$parameter = (Get-Command Get-D365ODataMetadata).Parameters['Path']
		...
		It 'Should have the expected parameter Url' {
			$parameter = (Get-Command Get-D365ODataMetadata).Parameters['Url']
		...
		It 'Should have the expected parameter Tenant' {
			$parameter = (Get-Command Get-D365ODataMetadata).Parameters['Tenant']
		...
		It 'Should have the expected parameter ClientId' {
			$parameter = (Get-Command Get-D365ODataMetadata).Parameters['ClientId']
		...
		It 'Should have the expected parameter ClientSecret' {
			$parameter = (Get-Command Get-D365ODataMetadata).Parameters['ClientSecret']
		...
		It 'Should have the expected parameter Token' {
			$parameter = (Get-Command Get-D365ODataMetadata).Parameters['Token']
		...
		It 'Should have the expected parameter EnableException' {
			$parameter = (Get-Command Get-D365ODataMetadata).Parameters['EnableException']
	...
	Describe "Testing parameterset __AllParameterSets" {
		...
		__AllParameterSets -Path -Url -Tenant -ClientId -ClientSecret -Token -EnableException
		#>
	}
