Describe "Remove-D365ODataEntity Unit Tests" -Tag "Unit" {
	...
	Describe "Ensuring unchanged command signature" {
		...
		It 'Should have the expected parameter RetryTimeout' {
			$parameter = (Get-Command Remove-D365ODataEntity).Parameters['RetryTimeout']
		...
		It 'Should have the expected parameter ThrottleSeed' {
			$parameter = (Get-Command Remove-D365ODataEntity).Parameters['ThrottleSeed']
		...
		It 'Should have the expected parameter SystemUrl' {
			$parameter = (Get-Command Remove-D365ODataEntity).Parameters['SystemUrl']
		...
		It 'Should have the expected parameter Token' {
			$parameter = (Get-Command Remove-D365ODataEntity).Parameters['Token']
	...
	Describe "Testing parameterset __AllParameterSets" {
		...
		__AllParameterSets -EntityName -Key -CrossCompany -RetryTimeout -ThrottleSeed -Tenant -Url -SystemUrl -ClientId -ClientSecret -Token -EnableException
		#>
	}
