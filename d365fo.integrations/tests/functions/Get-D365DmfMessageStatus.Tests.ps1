Describe "Get-D365DmfMessageStatus Unit Tests" -Tag "Unit" {
	BeforeAll {
	...
	Describe "Ensuring unchanged command signature" {
		It "should have the expected parameter sets" {
			(Get-Command Get-D365DmfMessageStatus).ParameterSets.Name | Should -Be '__AllParameterSets'
		}
		
		It 'Should have the expected parameter MessageId' {
			$parameter = (Get-Command Get-D365DmfMessageStatus).Parameters['MessageId']
			$parameter.Name | Should -Be 'MessageId'
		...
		It 'Should have the expected parameter Tenant' {
			$parameter = (Get-Command Get-D365DmfMessageStatus).Parameters['Tenant']
		...
		It 'Should have the expected parameter Url' {
			$parameter = (Get-Command Get-D365DmfMessageStatus).Parameters['Url']
		...
		It 'Should have the expected parameter SystemUrl' {
			$parameter = (Get-Command Get-D365DmfMessageStatus).Parameters['SystemUrl']
		...
		It 'Should have the expected parameter ClientId' {
			$parameter = (Get-Command Get-D365DmfMessageStatus).Parameters['ClientId']
		...
		It 'Should have the expected parameter ClientSecret' {
			$parameter = (Get-Command Get-D365DmfMessageStatus).Parameters['ClientSecret']
		...
		It 'Should have the expected parameter WaitForCompletion' {
			$parameter = (Get-Command Get-D365DmfMessageStatus).Parameters['WaitForCompletion']
		...
		It 'Should have the expected parameter Token' {
			$parameter = (Get-Command Get-D365DmfMessageStatus).Parameters['Token']
		...
		It 'Should have the expected parameter EnableException' {
			$parameter = (Get-Command Get-D365DmfMessageStatus).Parameters['EnableException']
	...
	Describe "Testing parameterset __AllParameterSets" {
		<#
		__AllParameterSets -MessageId
		__AllParameterSets -MessageId -Tenant -Url -SystemUrl -ClientId -ClientSecret -WaitForCompletion -Token -EnableException
		#>
	}
