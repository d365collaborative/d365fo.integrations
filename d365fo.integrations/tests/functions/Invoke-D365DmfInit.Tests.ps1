Describe "Invoke-D365DmfInit Unit Tests" -Tag "Unit" {
	BeforeAll {
	...
	Describe "Ensuring unchanged command signature" {
		It "should have the expected parameter sets" {
			(Get-Command Invoke-D365DmfInit).ParameterSets.Name | Should -Be '__AllParameterSets'
		}
		
		It 'Should have the expected parameter Tenant' {
			$parameter = (Get-Command Invoke-D365DmfInit).Parameters['Tenant']
		...
		It 'Should have the expected parameter Url' {
			$parameter = (Get-Command Invoke-D365DmfInit).Parameters['Url']
		...
		It 'Should have the expected parameter SystemUrl' {
			$parameter = (Get-Command Invoke-D365DmfInit).Parameters['SystemUrl']
		...
		It 'Should have the expected parameter ClientId' {
			$parameter = (Get-Command Invoke-D365DmfInit).Parameters['ClientId']
		...
		It 'Should have the expected parameter ClientSecret' {
			$parameter = (Get-Command Invoke-D365DmfInit).Parameters['ClientSecret']
		...
		It 'Should have the expected parameter Token' {
			$parameter = (Get-Command Invoke-D365DmfInit).Parameters['Token']
		...
		It 'Should have the expected parameter EnableException' {
			$parameter = (Get-Command Invoke-D365DmfInit).Parameters['EnableException']
	...
	Describe "Testing parameterset __AllParameterSets" {
		<#
		__AllParameterSets -
		__AllParameterSets -Tenant -Url -SystemUrl -ClientId -ClientSecret -Token -EnableException
		#>
	}
