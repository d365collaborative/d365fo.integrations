Describe "Get-D365ODataEntityMandatoryFields Unit Tests" -Tag "Unit" {
	BeforeAll {
	...
	Describe "Ensuring unchanged command signature" {
		It "should have the expected parameter sets" {
			(Get-Command Get-D365ODataEntityMandatoryFields).ParameterSets.Name | Should -Be '__AllParameterSets'
		}
		
		It 'Should have the expected parameter Name' {
			$parameter = (Get-Command Get-D365ODataEntityMandatoryFields).Parameters['Name']
		...
		It 'Should have the expected parameter Properties' {
			$parameter = (Get-Command Get-D365ODataEntityMandatoryFields).Parameters['Properties']
